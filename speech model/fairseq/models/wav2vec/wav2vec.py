...
class Wav2Vec2Config(FairseqDataclass):
    ...
    normtype: str = field(
        default="layernorm",
        metadata={"help": "normalization type: layernorm, dyt, or derf"}
    )
...
def get_norm_layer(normtype, normalized_shape, alpha_init_value=0.5, shift_init_value=0.0):
    if normtype == "layernorm":
        return LayerNorm(normalized_shape)
    elif normtype == "dyt":
        return DynamicTanh(normalized_shape, alpha_init_value=alpha_init_value)
    elif normtype == "derf":
        return Dynamic_erf(normalized_shape, alpha_init_value=alpha_init_value, shift_init_value=shift_init_value)
    else:
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, args: Wav2Vec2Config, skip_pos_conv: bool = False, override_encoder_layer: int = None):
        ...
        self.layer_norm = get_norm_layer(args.normtype, self.embedding_dim)
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn_layer_norm = get_norm_layer(normtype, self.embedding_dim)
        self.fc1 = nn.Linear(self.embedding_dim, ffn_embedding_dim)
        self.fc2 = nn.Linear(ffn_embedding_dim, self.embedding_dim)

        # layer norm associated with the position wise feed-forward NN
        self.final_layer_norm = get_norm_layer(normtype, self.embedding_dim)
